...
$interceptor = new class {
    ...
    function __invoke($uri = null)
    {
        ...
        foreach ($routes as $n => $route) {
            ...
            Terminal::echo("[#method]:[#cyan:#call] [#response][#magenta:#middlewares]", $route);
            Terminal::echo(" [#blue:#file] [#yellow:#replaced]", $route);
        }
    }
    ...
    protected function organize(array $routes): array
    {
        ...
        foreach (array_reverse($routes) as $route) {
            ...
            if (!$routeScheme['replaced'])
                $used[$key] = $routeScheme['file'];
